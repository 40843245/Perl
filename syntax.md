...
##### dumpValue
To print more pretty, use `dumpValue` function.

> [!IMPORTANT]
> The `dumpValue` function always require ti pass by reference.

> [!NOTE]
> The `dumpValue` function is NOT a built-in function, it is located in `dumpvar.pl`.
...
will output like this with this format:

```
0  11.233
1  HASH(0x110cc8c)
   3 => 4
   'hello' => ARRAY(0x110c9cc)
      0  6
      1  7
```

To print more pretty, one also can use `pretty_print` function.

+ Example 1:
  
```
# use package that it needs.

@sample = (11.233,{3 => 4, "hello" => [6,7]});
pretty_print(@sample); # what happened? it is funny, isn't it?
```

The package `PrettyPrinter.pm` as follows.
...
It will output like this format:
